...
def update_analysis_batch(batch_map_path:str, db_file_path:str, parent_dir_path:str, id_cln_name:str, temp_target_path:str, found_tag:str):
    '''
    Extracts metadata from tree structutre of the output folder and adding it to the db file. 
    Search results are stored in batch_map files between the runs.
    '''
    batch_map = pd.read_csv(batch_map_path) #READ BATCH FILE
    db_df = pd.read_csv(db_file_path) #READ DATABASE FILE
    ...
    if search_list.empty:
        ...
        filtered_df = path_df[path_df[9].isin(search_list)][[7,9]] #KEEP INFORMATION ABOUT THE SAMPLES THAT MUST BE SEARCHED
