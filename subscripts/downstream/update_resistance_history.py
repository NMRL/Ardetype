...
def process_report(path_to_df:str, delimiter:str=',', id_column:str='sample_id') -> pd.DataFrame:
    """ 
    ...
    df[id_column] = df[id_column].str.replace('.amr.alignment', '')
    df[id_column] = df[id_column].str.replace(r"_S[0-9]*", '')
    return df
...
if __name__ == '__main__':
    ...
    res_df = process_report(os.path.abspath(parser.hamronization), id_column='input_file_name', delimiter='\t') 
    try:
        rfp_df = process_report(os.path.abspath(parser.resfinder))
    except:
    ...
    try:
        pf_df  = process_report(os.path.abspath(parser.pointfinder))
    except: 
        pf_df = pd.DataFrame(columns = [
            "sample_id","analysis_batch_id","Mutation","Nucleotide change","Amino acid change","Resistance","PMID"
        ])
    try:
        amfm_df  = process_report(os.path.abspath(parser.amrfinder_mut), )
    except: 
