...
req_arg_grp.add_argument('-d', '--db', metavar='\b', help = 'Database to use with kmerfinder (bacteria, archea, fungi, protozoa, viral, typestrain)', default=None, required=True)
...
seq_id_list = list(f.split("/")[-1][:len(f)-16] for f in file_list) #UNIQUE IDS EXTRACTED FROM EACH FILE
seq_id_list = set([f[:len(f)-16] for f in seq_id_list])
...
def submit_job(sample_id,que):
    ...
    subprocess.check_call(['qsub', '-F', f'{read_1} {read_2} {sample_id} {database}', 'kmerfinder_job.sh'])
...
while len(file_dict.keys()) < len(read_dict.keys()):
    ...
    print(f'Waiting for the processing to complete for {sleep_time} seconds.')
