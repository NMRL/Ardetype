...
#sistr
singularity run ${sistr_path} sistr --qc -f csv ${contings} -o ~/salmonella_sra/sistr/${sample_id::-6}_sistr.csv ${contigs}
# --qc - check data quality - informative messages regarding missing loci, genes, issues with genome sizes or else
# -f output format

#seqsero2
singularity run ${seqsero2_path} SeqSero2_package.py -d ~/salmonella_sra/seqsero2/${sample_id::-6} -n ${sample_id::-6} -p 16 -t 2 -i ${read_1} ${read_2}
# -t paired-end reads
# -n sample name
# -p threads
# -d directory name
